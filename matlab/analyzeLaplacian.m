...
% Load the data which was generated by the laplacian clustering. The points
% are: 
% - 100 points from a Laplacian at [0,0] with sigma = I
% - 100 points from a Laplacian at [4,0] with sigma = [2 0; 0 0.5] 
% The points are generated equally from each, with the true clustering
% being in the order loaded (i.e first 100 points are one cluster, second
% 100 points are another).
load('lapData.mat');
...
lm = lapStruct{1}.lm;
points = lapStruct{1}.points;

% Calculate the true partition (i.e cluster values) of each of the points.
% We know the order in which they were sampled, so this is easy.
firstCluster = 1 * ones(100,1);
secondCluster = 2 * ones(100,1);
truePartition = cat(1,firstCluster,secondCluster);
...
for i = 1:size(lapStruct,2)
    ...
    if ~isfield(resultStruct, 'kinkBeta') || ...
      ...
      redoKinkBeta
        ...
        % Make note that we changed the lapStruct if we are going to save
        % it (in the kink angle case)
        if ~useInputBeta
            lapStruct{i} = resultStruct;
            changed = true;
        end
    ...
    if i == 1
        ...
        ylabel('Y');
        zlabel('PDF');
        title('True PDF of Non-Symmetric GM at (0,0) + (4,0)');
        ...
        fcontour(@(x,y) lm([x y]),...
            [min(X(:,1)) max(X(:,1)) min(X(:,2)) max(X(:,2))]);
        % Show the points with their true clusters. We know the clusters
        % are ordered by the first 100 and the second 100 points.
        gscatter(points(:,1),points(:,2),truePartition)
        hold off;
        xlabel('X');
        ylabel('Y');
        ...
        ylabel('Y');
        zlabel('f(x,y)');
        title('GM Distribution of gaussians centered at each of the 200 x_i');
    end
